...
class ListKidsFollow(View):
    ...
    def post(self,request,*args, **kwargs):
        print('probando len', len(request.POST['mes']))
        json_data4 = []
        ...
        print('IMPRIMIENDO MES', mes)

        if request.POST['red'] == 'TODOS':
            total = PackChildFollow.objects.filter(fec_nac__gte=request.POST['anio']+'-'+mes+'-01').aggregate(total=Count('id'))['total']
            cumplen = PackChildFollow.objects.filter(fec_nac__gte=request.POST['anio']+'-'+mes+'-01', mide=1).aggregate(cumplen=Count('id'))['cumplen']
            ...
            dataProv = PackChildFollow.objects.filter(fec_nac__gte=request.POST['anio']+'-'+mes+'-01').values('provincia').annotate(den=Count('id'),
                    num=Sum(Case(When(mide=1, then=1), default=0, output_field=IntegerField())),
                    avance=(ExpressionWrapper( Cast(Sum(Case(When(mide=1, then=1), default=0, output_field=IntegerField())), FloatField()) /
                    Cast(Count('*'), FloatField()) * 100, output_field=FloatField()))).order_by('-avance', '-den', '-num')
            dataDist = PackChildFollow.objects.filter(fec_nac__gte=request.POST['anio']+'-'+mes+'-01').values('provincia', 'distrito').annotate(den=Count('id'),
                    num=Sum(Case(When(mide=1, then=1), default=0, output_field=IntegerField())),
                    avance=(ExpressionWrapper( Cast(Sum(Case(When(mide=1, then=1), default=0, output_field=IntegerField())), FloatField()) /
                    Cast(Count('*'), FloatField()) * 100, output_field=FloatField()))).order_by('-avance', '-den', '-num')
            dataNom = PackChildFollow.objects.filter(fec_nac__gte=request.POST['anio']+'-'+mes+'-01')
            ...
            total = PackChildFollow.objects.filter(cod_prov=request.POST['red'], fec_nac__gte=request.POST['anio']+'-'+mes+'-01').aggregate(total=Count('id'))['total']
            cumplen = PackChildFollow.objects.filter(cod_prov=request.POST['red'], fec_nac__gte=request.POST['anio']+'-'+mes+'-01', mide=1).aggregate(cumplen=Count('id'))['cumplen']
            dataTotal = { 'total': total, 'cumple': cumplen, 'avance': round((cumplen/total)*100, 1) if total != 0 else 0 }
            dataProv = PackChildFollow.objects.filter(cod_prov=request.POST['red'], fec_nac__gte=request.POST['anio']+'-'+mes+'-01').values('provincia').annotate(den=Count('id'),
                        num=Sum(Case(When(mide=1, then=1), default=0, output_field=IntegerField())),
                        avance=(ExpressionWrapper( Cast(Sum(Case(When(mide=1, then=1), default=0, output_field=IntegerField())), FloatField()) /
                        Cast(Count('*'), FloatField()) * 100, output_field=FloatField()))).order_by('-avance', '-den', '-num')
            dataDist = PackChildFollow.objects.filter(cod_prov=request.POST['red'], fec_nac__gte=request.POST['anio']+'-'+mes+'-01').values('provincia', 'distrito').annotate(den=Count('id'),
                        num=Sum(Case(When(mide=1, then=1), default=0, output_field=IntegerField())),
                        avance=(ExpressionWrapper( Cast(Sum(Case(When(mide=1, then=1), default=0, output_field=IntegerField())), FloatField()) /
                        Cast(Count('*'), FloatField()) * 100, output_field=FloatField()))).order_by('-avance', '-den', '-num')
            dataNom = PackChildFollow.objects.filter(cod_prov=request.POST['red'], fec_nac__gte=request.POST['anio']+'-'+mes+'-01')
            dataNom = json.loads(serializers.serialize('json', dataNom, indent=2, use_natural_foreign_keys=True))

        elif request.POST['red'] != 'TODOS' and request.POST['dist'] != 'TODOS':
            total = PackChildFollow.objects.filter(cod_dist=request.POST['dist'], fec_nac__gte=request.POST['anio']+'-'+mes+'-01').aggregate(total=Count('id'))['total']
            cumplen = PackChildFollow.objects.filter(cod_dist=request.POST['dist'], fec_nac__gte=request.POST['anio']+'-'+mes+'-01', mide=1).aggregate(cumplen=Count('id'))['cumplen']
            dataTotal = { 'total': total, 'cumple': cumplen, 'avance': round((cumplen/total)*100, 1) if total != 0 else 0 }
            dataProv = PackChildFollow.objects.filter(cod_dist=request.POST['dist'], fec_nac__gte=request.POST['anio']+'-'+mes+'-01').values('provincia').annotate(den=Count('id'),
                        num=Sum(Case(When(mide=1, then=1), default=0, output_field=IntegerField())),
                        avance=(ExpressionWrapper( Cast(Sum(Case(When(mide=1, then=1), default=0, output_field=IntegerField())), FloatField()) /
                        Cast(Count('*'), FloatField()) * 100, output_field=FloatField()))).order_by('-avance', '-den', '-num')
            dataDist = PackChildFollow.objects.filter(cod_dist=request.POST['dist'], fec_nac__gte=request.POST['anio']+'-'+mes+'-01').values('provincia', 'distrito').annotate(den=Count('id'),
                        num=Sum(Case(When(mide=1, then=1), default=0, output_field=IntegerField())),
                        avance=(ExpressionWrapper( Cast(Sum(Case(When(mide=1, then=1), default=0, output_field=IntegerField())), FloatField()) /
                        Cast(Count('*'), FloatField()) * 100, output_field=FloatField()))).order_by('-avance', '-den', '-num')
            dataNom = PackChildFollow.objects.filter(cod_dist=request.POST['dist'], fec_nac__gte=request.POST['anio']+'-'+mes+'-01')
            dataNom = json.loads(serializers.serialize('json', dataNom, indent=2, use_natural_foreign_keys=True))
...
class ReportPackChildExcel(TemplateView):
    def get(self, request, *args, **kwargs):
        print(request.GET['red'], request.GET['dist'], request.GET['anio'], request.GET['mes'])
        ...
        set_border(self, ws, "A2:BB2", "medium", "57267C")
        set_border(self, ws, "A4:BB4", "medium", "366092")
        set_border(self, ws, "A6:BB6", "medium", "D9D9D9")

        img = Image('static/img/logoPrint.png')
        ws.add_image(img, 'A2')

        ws.merge_cells('B2:BB2')
        ...
        ws['B2'] = 'ESSALUD: MC-02: Niñas y niños menores de 12 meses de edad, procedentes de los quintiles 1 y 2 de pobreza departamental que reciben el paquete integrado de servicios -' + request.GET['anio'] + ' de ' + nameMonth.upper() + ' A LA FECHA'

        ws.merge_cells('A4:BB4')
        ...
        ws['K8'].fill = PatternFill(start_color='B3F5C2', end_color='B3F5C2', fill_type='solid')
        ...
        ws['M8'].fill = PatternFill(start_color='DBCAF4', end_color='DBCAF4', fill_type='solid')
        ...
        ws['N8'].fill = PatternFill(start_color='FCE4D6', end_color='FCE4D6', fill_type='solid')
        ...
        ws['O8'].fill = PatternFill(start_color='E2EFDA', end_color='E2EFDA', fill_type='solid')
        ...
        ws['P8'].fill = PatternFill(start_color='F5F3CB', end_color='F5F3CB', fill_type='solid')
        ...
        ws['Q8'].fill = PatternFill(start_color='DDEBF7', end_color='DDEBF7', fill_type='solid')
        ...
        ws['S8'].fill = PatternFill(start_color='DBCAF4', end_color='DBCAF4', fill_type='solid')
        ...
        ws['T8'].fill = PatternFill(start_color='FFFFFF', end_color='FFFFFF', fill_type='solid')
        ...
        ws['U8'].fill = PatternFill(start_color='FCE4D6', end_color='FCE4D6', fill_type='solid')
        ...
        ws['V8'].fill = PatternFill(start_color='E2EFDA', end_color='E2EFDA', fill_type='solid')
        ...
        ws['W8'].fill = PatternFill(start_color='DDEBF7', end_color='DDEBF7', fill_type='solid')
        ...
        ws['X8'].fill = PatternFill(start_color='F5F3CB', end_color='F5F3CB', fill_type='solid')
        ...
        ws['Y8'].fill = PatternFill(start_color='DBCAF4', end_color='DBCAF4', fill_type='solid')
        ...
        ws['Z8'].fill = PatternFill(start_color='FFFFFF', end_color='FFFFFF', fill_type='solid')
        ...
        ws['AD8'].fill = PatternFill(start_color='FFFFFF', end_color='FFFFFF', fill_type='solid')
        ...
        ws['AE8'].fill = PatternFill(start_color='F5F3CB', end_color='F5F3CB', fill_type='solid')
        ...
        ws['AG8'].fill = PatternFill(start_color='DBCAF4', end_color='DBCAF4', fill_type='solid')
        ...
        ws['AH8'].fill = PatternFill(start_color='FFFFFF', end_color='FFFFFF', fill_type='solid')
        ...
        ws['AQ8'].fill = PatternFill(start_color='95B3D7', end_color='95B3D7', fill_type='solid')
        ...
        ws['BB8'].fill = PatternFill(start_color='FFFF00', end_color='FFFF00', fill_type='solid')
        ...
        if request.GET['red'] == 'TODOS':
            dataNom = PackChildFollow.objects.filter(fec_nac__gte=request.GET['anio']+'-'+mes+'-01').order_by('provincia','distrito')
            dataNom = json.loads(serializers.serialize('json', dataNom, indent=2, use_natural_foreign_keys=True))
        elif request.GET['red'] != 'TODOS' and request.GET['dist'] == 'TODOS':
            dataNom = PackChildFollow.objects.filter(cod_prov=request.GET['red'], fec_nac__gte=request.GET['anio']+'-'+mes+'-01').order_by('provincia','distrito')
            dataNom = json.loads(serializers.serialize('json', dataNom, indent=2, use_natural_foreign_keys=True))
        elif request.GET['red'] != 'TODOS' and request.GET['dist'] != 'TODOS':
            dataNom = PackChildFollow.objects.filter(cod_dist=request.GET['dist'], fec_nac__gte=request.GET['anio']+'-'+mes+'-01').order_by('provincia','distrito')
            dataNom = json.loads(serializers.serialize('json', dataNom, indent=2, use_natural_foreign_keys=True))
        ...
        if cant > 0:
            for paqNinio in dataNom:
                ...
                ws.cell(row=cont, column=6).value = paqNinio['fields']['fec_nac']
                ws.cell(row=cont, column=7).value = paqNinio['fields']['ctrl1rn']
                ws.cell(row=cont, column=8).value = paqNinio['fields']['ctrl2rn']
                ws.cell(row=cont, column=9).value = paqNinio['fields']['ctrl3rn']
                ws.cell(row=cont, column=10).value = paqNinio['fields']['ctrl4rn']
                ...
                ws.cell(row=cont, column=12).value = paqNinio['fields']['cred1']
                ws.cell(row=cont, column=13).value = paqNinio['fields']['cred2']
                ws.cell(row=cont, column=14).value = paqNinio['fields']['neumo2']
                ws.cell(row=cont, column=15).value = paqNinio['fields']['rota2']
                ws.cell(row=cont, column=16).value = paqNinio['fields']['polio2']
                ws.cell(row=cont, column=17).value = paqNinio['fields']['penta2']
                ws.cell(row=cont, column=18).value = paqNinio['fields']['cred3']
                ws.cell(row=cont, column=19).value = paqNinio['fields']['cred4']
                ws.cell(row=cont, column=20).value = paqNinio['fields']['suple4']
                ws.cell(row=cont, column=21).value = paqNinio['fields']['neumo4']
                ws.cell(row=cont, column=22).value = paqNinio['fields']['rota4']
                ws.cell(row=cont, column=23).value = paqNinio['fields']['penta4']
                ws.cell(row=cont, column=24).value = paqNinio['fields']['polio4']
                ws.cell(row=cont, column=25).value = paqNinio['fields']['cred5']
                ws.cell(row=cont, column=26).value = paqNinio['fields']['suple5']
                ws.cell(row=cont, column=27).value = paqNinio['fields']['cred6']
                ws.cell(row=cont, column=28).value = paqNinio['fields']['tmz']
                ws.cell(row=cont, column=29).value = paqNinio['fields']['dxAnemia']
                ws.cell(row=cont, column=30).value = paqNinio['fields']['suple6']
                ws.cell(row=cont, column=31).value = paqNinio['fields']['polio6']
                ws.cell(row=cont, column=32).value = paqNinio['fields']['penta6']
                ws.cell(row=cont, column=33).value = paqNinio['fields']['cred7']
                ws.cell(row=cont, column=34).value = paqNinio['fields']['suple7']
                ws.cell(row=cont, column=35).value = paqNinio['fields']['cred8']
                ws.cell(row=cont, column=36).value = paqNinio['fields']['suple8']
                ws.cell(row=cont, column=37).value = paqNinio['fields']['cred9']
                ws.cell(row=cont, column=38).value = paqNinio['fields']['suple9']
                ws.cell(row=cont, column=39).value = paqNinio['fields']['cred10']
                ws.cell(row=cont, column=40).value = paqNinio['fields']['suple10']
                ws.cell(row=cont, column=41).value = paqNinio['fields']['cred11']
                ws.cell(row=cont, column=42).value = paqNinio['fields']['suple11']
                ws.cell(row=cont, column=43).value = paqNinio['fields']['eval_oral']
                ws.cell(row=cont, column=44).value = paqNinio['fields']['dif1']
                ws.cell(row=cont, column=44).alignment = Alignment(horizontal="center")
                ws.cell(row=cont, column=45).value = paqNinio['fields']['dif2']
                ws.cell(row=cont, column=45).alignment = Alignment(horizontal="center")
                ws.cell(row=cont, column=46).value = paqNinio['fields']['dif3']
                ws.cell(row=cont, column=46).alignment = Alignment(horizontal="center")
                ws.cell(row=cont, column=47).value = paqNinio['fields']['dif4']
                ws.cell(row=cont, column=47).alignment = Alignment(horizontal="center")
                ws.cell(row=cont, column=48).value = paqNinio['fields']['dif5']
                ws.cell(row=cont, column=48).alignment = Alignment(horizontal="center")
                ws.cell(row=cont, column=49).value = paqNinio['fields']['dif6']
                ws.cell(row=cont, column=49).alignment = Alignment(horizontal="center")
                ws.cell(row=cont, column=50).value = paqNinio['fields']['dif7']
                ws.cell(row=cont, column=50).alignment = Alignment(horizontal="center")
                ws.cell(row=cont, column=51).value = paqNinio['fields']['dif8']
                ws.cell(row=cont, column=51).alignment = Alignment(horizontal="center")
                ws.cell(row=cont, column=52).value = paqNinio['fields']['dif9']
                ws.cell(row=cont, column=52).alignment = Alignment(horizontal="center")
                ws.cell(row=cont, column=53).value = paqNinio['fields']['dif10']
                ws.cell(row=cont, column=53).alignment = Alignment(horizontal="center")

                if paqNinio['fields']['mide'] == '1':
                    cumplen = '✔'
                    ws.cell(row=cont, column=54).font = Font(name='Calibri', size=10, color='00AC4E')
                else:
                    cumplen = '✘'
                    ws.cell(row=cont, column=54).font = Font(name='Calibri', size=10, color='C00000')

                ws.cell(row=cont, column=54).alignment = Alignment(horizontal="center")
                ws.cell(row=cont, column=54).value = cumplen
